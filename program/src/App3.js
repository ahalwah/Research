...
export default function App3() {
  ...
  useEffect(() => {
    ...
    // controls
    const orbitControls = new OrbitControls(orbitCamera, output_canvas.current); // renderer.domElement
    ...
    const rigRotation = (
      ...
      const Part = currentVrm.humanoid.getBoneNode(
        VRM.VRMSchema.HumanoidBoneName[name]
      );
      if (!Part) {
        return;
      }
    ...
    const rigFace = (riggedFace) => {
      ...
      const PresetName = VRM.VRMSchema.BlendShapePresetName;
    ...
    let videoElement = input_video.current,
      guideCanvas = video_canvas.current;
